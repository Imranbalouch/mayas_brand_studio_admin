document.title = "Dashboard | Countries";
let PageNo = 1;
let PageSize = 10;
let totalRecord = 0;
let PageLength = 0;
...
function Onload() {
    ...
    $('#buttons').html('');
    $.ajax({
        url: ApiPlugin + '/ecommerce/get_country',
        type: "Get",
                                ...
                                if (row.image != null) {
                                    return `
                                        <div class="d-flex gap-2 align-items-center product-desc-tb btn-edit">
                                            <div><img style="min-width: 48px;"src="${AssetsPath + row.image}" width="48" alt="image"></div>
                                        </div>`;
                                } else {
                                    return '-';
                                }
...
$(document).on('click', '.btn-edit', function (e) {
    e.preventDefault();
    $("#popup_title").html(getTranslation("edit_supplier"));
...
$(document).on('click', '.btn-delete', function (e) {
    ...
    Swal.fire({
        ...
        if (result.value) {
            $.ajax({
                url: ApiPlugin + "/ecommerce/delete_country/" + uuid,
                type: "delete",
                data: {},
                beforeSend: function (xhr) {
                    xhr.setRequestHeader("Authorization", "Bearer " + strkey);
...
function changestatus(el) {
    ...
    $.ajax({
        url: ApiPlugin + "/ecommerce/status_country/" + $(el).val(),
        type: "POST",
        data: { status: status },
        beforeSend: function (xhr) {
            xhr.setRequestHeader("Authorization", "Bearer " + strkey);
