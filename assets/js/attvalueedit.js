document.title="Dashboard | Edit Attribute Values";
var urlParams = new URLSearchParams(window.location.search);
var id = urlParams.get('id');
if (id) {
    $.ajax({
        url: ApiPlugin + "/ecommerce/attribute/edit_specific_attribute_value/" + id,
        type: "GET",
        dataType: "json",
        ...
        error: function (error) {
            ...
            if (error.status_code == 404) {
                showToast(error.message, 'Error', 'error', '?P=attribute&M=' + menu_id + '&id=' + id);
            } else {
                showToast('Internal Server Error.', 'Error', 'error', '?P=attribute&M=' + menu_id + '&id=' + id);
            }
        }
    });
}
...
/// edit form submit
$("#editAttributeValueForm").on("submit", function (e) {
    ...
    imgload.show();
    $.ajax({
        url: ApiPlugin + "/ecommerce/attribute/update_attribute_value",
        type: "POST",
        data: formData,
        contentType: false,
        processData: false,
        ...
        success: function (response) {
            ...
            if (response.status_code == 200) {
                ...
                showToast(title, 'Success', 'success', '?P=attribute&M=' + menu_id + '&id=' + id);
            }
        },
