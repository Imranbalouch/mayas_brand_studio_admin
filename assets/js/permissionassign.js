document.title="Dashboard | Assign Permission";
var PageNo = 1;
var PageSize = 10;
var totalRecord = 0;
var PageLength = 0;
...
$("#cmb_role_id").on("change", function () { 
    role_id = this.value; 
    if (role_id != '') { 
          Onload(role_id);  
    }
...
$("#cmb_role_id_copy").on("change", function () { 
    role_id = this.value; 
    if (role_id != '') { 
          Onload(role_id);  
    }
...
function FillRoles() {   
    $.ajax({
        ...
        success: function(data) { 
            ...
            // Update selectpicker
             $('#cmb_role_id').html(options);
             $('#cmb_role_id_copy').html(blnk+options);
            // Refresh selectpicker to apply new options
            $('#cmb_role_id').selectpicker('refresh');
            $('#cmb_role_id_copy').selectpicker('refresh');
        },
...
function check_value(role_id,mmenu_id,pm_id,permissionAssign){ 
    if(role_id=="" || mmenu_id== "" || pm_id== ""||permissionAssign==null){
        return false;
    }else{ 
...
function checkall(p_id){ 
   ...
   function Onload(role_id) {  
    $('#buttons').html('');
  //console.log(role_id);
    $.ajax({
        url: ApiForm + '/get_permission_assign',
        type: "Get",
        contentType: "application/json",
        dataType: "json",
        ...
        beforeSend: function (xhr) {
            ...
            xhr.setRequestHeader("role-id",role_id); 
            imgload.show();
        },
                        ...
                        for (var pd_sno = 0; pd_sno < permission_menus.length; pd_sno++) {
                            ...
                            li_row +=  '<tr>'+
                                ...
                                for (var ck_sno = 0; ck_sno < permissions.length; ck_sno++) {
                                    ...
                                    var checkbox_value=check_value(role_id,mmenu_id,pm_id,permissionAssign); 
                                    ...
                                    li_row += '<td style="height:30px; text-align:center;font-weight:bold;"><input type="checkbox" class="form-check-input input_checkbox checkmark_' + pm_id + '"  '+check+'  id="ck_check_'+mmenu_id+'_' + pm_id + '" name="ck_check"  data-role-id="'+role_id +'" data-menu-id="'+ mmenu_id +'" data-permission-id="'+ pm_id +'"></td>';
                                }
                              li_row += '</tr>';
                        }
                    }
...
function update_record() {
    ...
    Swal.fire({
        ...
        if (result.value) {
            $.ajax({
                url: ApiForm + '/update_permission_assign',
                type: "Post",
                processData: false,
                contentType: false,
                data: cre,
                ...
                success: function (response) { 
                    if (response.status_code == 200) { 
                        ...
                        showToast(getTranslation('Permission updated successfully'), 'Success', 'success');
                    }  
                },
                error: function (xhr, status, err) {  
                    imgload.hide();
...
function ckvalidation() {  
        ...
        checkboxes.forEach(checkbox => {
          results.push({
           ...
           // status: checkbox.checked,
            status: checkbox.checked,
           // role_id: checkbox.getAttribute('data-role-id'),
    ...
    return { role_id: role_id, creteria: formData };
}
