document.title = "Dashboard | Carriers";
let PageNo = 1;
let PageSize = 10;
let totalRecord = 0;
let PageLength = 0;
...
function Onload() {
    ...
    $('#buttons').html('');
    $.ajax({
        url: ApiPlugin + '/ecommerce/carriers',
        type: "Get",
...
$(document).on('click', '.btn-delete', function (e) {
    ...
    Swal.fire({
        ...
        if (result.value) {
            $.ajax({
                url: ApiPlugin + "/ecommerce/carriers/delete/" + uuid,
                type: "delete",
                data: {},
                beforeSend: function (xhr) {
                    xhr.setRequestHeader("Authorization", "Bearer " + strkey);
...
function changestatus(el) {
    ...
    $.ajax({
        url: ApiPlugin + "/ecommerce/carriers/status/" + $(el).val(),
        type: "POST",
        data: { status: status },
        beforeSend: function (xhr) {
            xhr.setRequestHeader("Authorization", "Bearer " + strkey);
