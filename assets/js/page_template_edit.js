document.title = "Dashboard | Edit Page Template";
// Global variable to store variants
let VariantObj = {};

$(document).ready(function () {
    // Get the template UUID from URL parameters
    const urlParams = new URLSearchParams(window.location.search);
    const templateUuid = urlParams.get('id');

    if (!templateUuid) {
        showToast("Template UUID not found in URL", 'Error', 'error');
        return;
    }

    // Fetch template data
    fetchTemplateData(templateUuid);

});
// Set up the slug input change handler
$('#product-slug-input').on('change', function () {
    var pageType = $("#template_page_type").val();
...
function fetchTemplateData(uuid) {
    $.ajax({
        url: ApiCms + '/theme/page-template/edit/' + uuid,
        type: "GET",
        ...
        success: function (response) {
            if (response.status_code === 200 && response.data) {
                ...
                // Initialize variables based on page type
                updateInputValues(response.data.page_type);
                
                // If product detail, extract slug from API URL if possible
                if (response.data.page_type === 'product_detail') {
                    
                    const productSlug = response.data.api_url;
                    
                    if (productSlug) {
                        $('#basic-url').val(productSlug).show();
                        getdetailProduct(productSlug,ProductBaseApi,$('#basic-url').parent());
                        updateProductDetailVariables(productSlug);
                    }
                } else if (response.data.page_type === 'cart') {
                    $('#cart-slider-section').removeClass('d-none');
                    const slugMatch = response.data.api_url;
                    getdetailProduct(slugMatch,BaseApi,$('#basic-url').parent());
                    updateVariablesForPageType('cart');
                }else{
                    $('#cart-slider-section').addClass('d-none');
                    const slugMatch = response.data.api_url;
                    if (slugMatch) {
                        getdetailProduct(slugMatch,BaseApi,$('#basic-url').parent());
                        updateVariablesForPageType(response.data.page_type);
                    }
                }
                // // Load html_variant if exists
                // if (response.data.html_variant) {
                //     try {
                //         // Load saved variant data
                //         VariantObj = JSON.parse(response.data.html_variant);
                //         $("#html_variant").val(response.data.html_variant);
                //     } catch (e) {
                //         console.error("Error parsing html_variant:", e);
                //     }
                // }
            } else {
                showToast("Failed to load template data", 'Error', 'error');
            }
            imgload.hide();
        },
        error: function (xhr, status, error) {
            imgload.hide();
            showToast("Error fetching template data", 'Error', 'error');
        }
    });
}
...
function updateProductDetailVariables(slug) {
    var apiUrl = ProductBaseApi + slug;
    $("#template_api_url").text(ProductBaseApi);
...
// // Attach copy functionality to copy buttons
// $(document).on('click', 'button[onclick="copyInputText(this)"]', function () {
//     copyInputText(this);
// });

// // Form submission handler for update
$("#editProductDesignForm").submit(function (e) {
    e.preventDefault();

    if ($("#template_page_type").val() === '') {
        showToast("Please select page type", 'Error', 'error');
        return false;
    }

    var formData = new FormData(this);
    formData.append('cart_section_list', $('#cart_section_list').val());
    formData.append('cart_section_list_empty', $('#cart_section_list_empty').val());
    formData.append('product_cart_list', $('#product_cart_list').val());
    formData.append('product_cart_subtotal_list', $('#product_cart_subtotal_list').val());
    formData.append('product_cart_total_list', $('#product_cart_total_list').val());

    // Add html_variant to form data
    if ($("#template_page_type").val() === 'product_detail' || $("#template_page_type").val() === 'order' || $("#template_page_type").val() === 'order_detail' || $("#template_page_type").val() === 'track_order') {
        formData.append('html_variant', $("#html_variant").val());
    }
    
    $.ajax({
        url: ApiCms + "/theme/page-template/update/" + $("#template_uuid").val(),
        type: "POST",
        data: formData,
        contentType: false,
        processData: false,
        ...
        error: function (xhr, status, error) {
            imgload.hide();
            let errorMessage = xhr.responseJSON && xhr.responseJSON.message ? xhr.responseJSON.message : "An error occurred while updating the page template.";
...
// Function to populate form with template data
function populateForm(data) {
    $("#template_uuid").val(data.uuid);
    $("#template_name").val(data.name);
    $("#product-slug-input").val(data.product_slug);
    if (data.page_type) {
        $("#template_page_type").val(data.page_type);
    }
    $("#template_api_url").text(BaseApi);
    $("input[name=api_url]").val(data.api_url);
    $("#template_product_cart_html").val(data.product_cart_html);
    $("#template_product_cart_slider_html").val(data.product_cart_slider_html);
    $("#template_page_html").val(data.page_html);
    $("textarea[name='html_variant']").val(data.html_variant);
    // Set product class if exists
    if (data.product_class) {
        $("input[name=product_class]").val(data.product_class);
    }
}
...
function getdetailProduct(slug, BaseApi,label = false) {
    ...
    return $.ajax({
        ...
        success: function (response) {
            ...
            if (pageType === 'cart' || pageType === 'checkout' || pageType === 'thankyou' || pageType === 'address' || pageType === 'login' || pageType === 'signup' || pageType === 'forget_password' || pageType === 'reset_password' || pageType === 'wishlist') {
               ...
               if (response.data != '' && response.data != null) {
                    ...
                    lih += '<div class="list-group-item copy-variable"><button type="button" data-id="order" onclick="variableModal(this)">Use</button><div class="row">';
            ...
            }else if(pageType === 'order_detail' || pageType === 'track_order'){
               if (response.data != '' && response.data != null) {
                    let orders = response.data;
                    let orderDetail = response.order_details;
                    let lih = '';
                    let li = '';
                    let subli = '';
                    let subhli = '';
                    ...
                    lih += '<div class="list-group-item copy-variable"><div class="row"><h4 class="mb-3">Order</h4>';
                    for (let i = 0; i < orders.length; i++) {
                        li += '<div class="col-3 mb-3 sub-variable"><div class="list-group-item copy-variable">${item.' + orders[i] + '}</div></div>';
                    }
                    lih += li + '</div></div>';
                    
                    subhli += '<div class="list-group-item copy-variable"><button type="button" data-id="order" onclick="variableModal(this)">Use</button><div class="row">';
                    for (let i = 0; i < orderDetail.length; i++) {
                        subli += '<div class="col-3 mb-3 sub-variable"><div class="list-group-item copy-variable">${item.' + orderDetail[i] + '}</div></div>';
                    }
                    subhli += subli + '</div></div>';
...
$("#variant_loop").submit(function (e) {
    ...
    let VariantObj = JSON.parse($("#html_variant").val());
    // Update VariantObj
