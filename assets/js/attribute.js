document.title="Dashboard | Attribute";
let PageNo = 1;
let PageSize = 10;
let totalRecord = 0;
let PageLength = 0;
...
function Onload() {
    ...
    $('#buttons').html('');
    $.ajax({
        url: ApiPlugin + '/ecommerce/attribute/get_attribute',
        type: "Get",
        ...
        success: function (response) {
            if (response.status_code == 200) {
                ...
                // Attribute Values
                if (Boolean(response.permissions["attributevalues"])) {
                    action_button += `<a href='javascript:;' class='modify_btn btn-attributevalues' data-toggle='tooltip' title='Attributes Values'>${getTranslation('attributes_values')}</a> `;
                }
                ...
                if (response["data"] != null) {
                    ...
                    const columnsConfig = [
                        ...
                        },
                        {
                            ...
                            render: function (data, type, row) {
                                if (row.attribute_values && row.attribute_values.length > 0) {
                                    return row.attribute_values.map(val => `<span style="background-color: #d5d5d5;padding: 5px;border-radius: 5px;">${val.value}</span>`).join(', ');
                                } else {
                                    return '-';
                                }
                            },
                            ...
                            title: 'Values' // New column title
                        },
                        {
                            data: null, // For 'color code' column
                            render: function (data, type, row) {
                                if (row.attribute_values && row.attribute_values.length > 0) { 
                                    return row.attribute_values
                                    .filter(val => val.color_code !== null) // Skip null values
                                    .map(val => `<span style="background-color: #d5d5d5; padding: 5px; border-radius: 5px;">${val.color_code}</span>`)
                                    .join(', ') || '-'; // If no valid color codes, return '-'
                            } else {
                                return '-';
                            }
                            },
                            createdCell: function (td) {
                                $(td).addClass('custom_span2');
                            },
                            title: 'Color Code' // New column title
                        },
                        {
                            data: 'status',
                            render: function (data, type, row) {
                            ...
                            details: {
                                display: $.fn.dataTable.Responsive.display.modal({
                                    header: function (row) {
                                        ...
                                        var attribute_name = data.attribute_name  ? data.attribute_name  : 'Unknown Attribute Name';
                                        return 'Details of ' + attribute_name;
                                    }
                                }),
                                type: 'column',
...
$(document).on('click', '.btn-delete', function (e) {
    ...
    Swal.fire({
        ...
        if (result.value) {
            $.ajax({
                url: ApiPlugin + "/ecommerce/attribute/delete_attribute/"+uuid,
                type: "delete",
                data: {},
                beforeSend: function (xhr) {
                    xhr.setRequestHeader("Authorization", "Bearer " + strkey);
