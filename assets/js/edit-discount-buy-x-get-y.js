document.title = "Dashboard | Edit Discount (Buy X Get Y)";
let uuid = '';
let edit_data = '';
...
$(document).ready(function () {
  ...
  $('#generateCode').click(function() {
    ...
    $('#applies_to').change(function() {
      ...
      console.log('applies_to changed to:', $(this).val(), 'productSelection visible:', $('#productSelection').is(':visible')); // Debug log
    });
  ...
  $('#collectionSearch').off('keyup').on('keyup', function() {
    ...
    $('#allCollectionsTable tbody tr').each(function() {
      ...
      $(this).toggle(collectionName.includes(searchTerm));
    });
  });

  $('#customerGetsCollectionSearch').on('keyup', function() {
  let searchTerm = $(this).val().toLowerCase();
 ...
 $('#collectionSearch').off('keyup').on('keyup', function() {
  ...
  $('#ProductItem, .custom-button-browse').on('click', function() {
    // Ensure product modal can be opened
    ...
    productArray.forEach(product => {
      tableBody.append(`
        <tr class="product-row" data-product-id="${product.product_uuid}">
          ...
          <td>${produto.variant || 'Base Product'}</td>
          <td>
  ...
  $('#addCustomerGetsCollections').on('click', function() {
    ...
    updateCustomerGetsSelectedCollectionsTable(selectedCollections);
    updateSummary();
  });

  // Add Customer Gets Products
  $('#AddCustomerGetsProductModal').on('click', function() {
    ...
    updateCustomerGetsSelectedProductsTable(productArray);
    updateSummary();
  });

  // Parent checkbox controls child checkboxes for Customer Gets
...
function initializeForm() {
    ...
    // Show collection or product selection based on applies_to
    ...
    // Ensure product modal button is always accessible
    $('#productSelection').find('button[data-bs-target="#exampleModalOne"]').show();
...
function populateForm(data) {
    ...
    $('#name').val(data.name);
    $('#code').val(data.code);
    $('#discount_type').val(data.discount_type) || 'buy_x_get_y';
    ...
    // Set customer buys
    $(`input[name="customer_buys"][value="${data.customer_buys}"]`).prop('checked', true).trigger('change');
    $('#customer_buys_quantity').val(data.customer_buys_quantity);
    $('#customer_buys_amount').val(data.customer_buys_amount);

    // Set customer gets
    $('#customer_get_quantity').val(data.customer_get_quantity);
    ...
    // Determine customer_gets_applies_to based on discount_items
    let customerGetsAppliesTo = 'collections'; // Default to collections
    if (data.discount_items) {
        if (data.discount_items.some(item => item.customer_get_product_id)) {
            customerGetsAppliesTo = 'specific';
        }
    }
    $('#customer_gets_applies_to').val(customerGetsAppliesTo).trigger('change');
    ...
    // Load customer gets items
    if (customerGetsAppliesTo === 'collections' && data.discount_items) {
        loadCustomerGetsCollectionsForEdit(data.discount_items);
    } else if (customerGetsAppliesTo === 'specific' && data.discount_items) {
        loadCustomerGetsProductsForEdit(data.discount_items);
    }
...
function populateCollectionsTable(collections) {
  ...
  collections.forEach(collection => {
    ...
    let isChecked = edit_data && edit_data.discount_items && 
                   edit_data.discount_items.some(item => item.customer_buy_collection_id === collection.uuid);
  ...
  if (edit_data && edit_data.applies_to === 'collections' && edit_data.discount_items) {
    ...
    edit_data.discount_items.forEach(item => {
      if (item.customer_buy_collection_id) {
        // Find the collection name from the collections array
        let collection = collections.find(c => c.uuid === item.customer_buy_collection_id);
        if (collection) {
          selectedCollections.push({
            uuid: item.customer_buy_collection_id,
    ...
    success: function(response) {
      ...
      if (response.status_code === 200) {
        ...
        discountItems.forEach(item => {
          if (item.customer_buy_product_id) {
            let product = products.find(p => p.uuid === item.customer_buy_product_id);
            if (product) {
              // For base product
              if (!item.customer_buy_variant_id) {
              ...
              // For variants
                ...
                let variant = product.product_stocks?.find(v => v.uuid === item.customer_buy_variant_id);
                if (variant) {
                  selectedProducts.push({
                    product_uuid: product.uuid,
...
function populateCustomerGetsCollectionsTable(collections) {
  ...
  // Get all collection IDs that are selected for Customer Gets
  let selectedCollectionIds = [];
  if (edit_data && edit_data.discount_items) {
    edit_data.discount_items.forEach(item => {
      if (item.customer_get_collection_id) {
        selectedCollectionIds.push(item.customer_get_collection_id);
      }
    });
  }
  
  collections.forEach(collection => {
    let isChecked = selectedCollectionIds.includes(collection.uuid);
  ...
  // If we have selected collections, update the selected collections table
  if (selectedCollectionIds.length > 0) {
    let selectedCollections = collections.filter(c => selectedCollectionIds.includes(c.uuid))
      .map(collection => ({
...
function updateCustomerGetsSelectedCollectionsTable(collections) {
...
function loadCustomerGetsCollectionsForEdit(discountItems) {
  $.ajax({
    url: ApiPlugin + '/ecommerce/collection/get_active_collections',
    type: 'GET',
    contentType: 'application/json',
    ...
    success: function(response) {
      ...
      if (response.status_code === 200) {
        ...
        // Get all collection items from discount data for customer gets
        discountItems.forEach(item => {
          if (item.customer_get_collection_id) {
            let collection = collections.find(c => c.uuid === item.customer_get_collection_id);
            if (collection) {
              selectedCollections.push({
                uuid: collection.uuid,
                name: collection.name
...
function populateCustomerGetsProductsTable(products) {
  ...
  // Get all product/variant IDs that are selected for Customer Gets
  let selectedProducts = {};
  if (edit_data && edit_data.discount_items) {
    edit_data.discount_items.forEach(item => {
      if (item.customer_get_product_id) {
        let key = `${item.customer_get_product_id}_${item.customer_get_variant_id || ''}`;
  ...
  // If we have selected products, update the selected products table
  if (Object.keys(selectedProducts).length > 0) {
    let selectedProductItems = [];
    // You'll need to implement this based on your product data structure
    // This would collect all selected products and variants
    
    updateCustomerGetsSelectedProductsTable(selectedProductItems);
  }
}

function loadCustomerGetsProductsForEdit(discountItems) {
  $.ajax({
    url: ApiPlugin + '/ecommerce/product/get_active_products',
    type: 'GET',
    contentType: 'application/json',
    ...
    success: function(response) {
      ...
      if (response.status_code === 200) {
        ...
        // Get all product items from discount data for customer gets
        discountItems.forEach(item => {
          if (item.customer_get_product_id) {
            let product = products.find(p => p.uuid === item.customer_get_product_id);
            if (product) {
              // For base product
              if (!item.customer_get_variant_id) {
                selectedProducts.push({
                  product_uuid: product.uuid,
                  variant_uuid: '',
                  name: product.name,
              ...
              // For variants
                ...
                let variant = product.product_stocks?.find(v => v.uuid === item.customer_get_variant_id);
                if (variant) {
                  selectedProducts.push({
                    product_uuid: product.uuid,
                    variant_uuid: variant.uuid,
        ...
        updateCustomerGetsSelectedProductsTable(selectedProducts);
      } else {
        showToast('Failed to load products', 'Error', 'error');
      }
    },
...
function updateCustomerGetsSelectedProductsTable(products) {
...
function populateCustomersTable(customers, selectedCustomerIds = []) {
  ...
  if (selectedCustomerIds.length > 0) {
    let selectedCustomers = customers.filter(c => selectedCustomerIds.includes(c.id.toString())).map(customer => ({
      id: customer.id,
      name: customer.name,
...
function updateSelectedCustomersTable(customers) {
  ...
  if (customers.length > 0) {
    $('#eligibility_value').val(customers.map(c => c.name).join(', '));
  }
}
    ...
    success: function (response) {
      ...
      if (response.status_code === 200) {
        ...
        if (edit_data && edit_data.discount_items) {
          edit_data.discount_items.forEach(item => {
            if (item.customer_buy_product_id) {
              let variantId = item.customer_buy_variant_id || '';
              selectedProducts[`${item.customer_buy_product_id}_${variantId}`] = true;
            }
          });
        }
...
function prepareDiscountData() {
  let discountData = {
    ...
    discount_type: $('#discount_type').val() || 'buy_x_get_y',
